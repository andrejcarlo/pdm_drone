...
class PRM:
    def __init__(self, numOfRandomCoordinates, obstacles, start, destination, goal=None):
        self.numOfCoords = numOfRandomCoordinates
        self.coordsList = np.array([])
        self.allObs = obstacles
        self.start = np.array(start)
        self.destination = np.array(destination)
        self.graph = Graph(start, destination)
    ...
    def sample_points(self):
        ...
        self.coordsList[:,0] = self.graph.searchbegin_x - self.coordsList[:,0]*self.graph.searchboxsize_x*2
        self.coordsList[:,1] = self.graph.searchbegin_y - self.coordsList[:,1]*self.graph.searchboxsize_y*2
        self.coordsList[:,2] = self.graph.searchbegin_z - self.coordsList[:,2]*self.graph.searchboxsize_z*2
