...
def bang_bang_velocity_profile(
    sampling_time, start_point, end_point, start_stop_speed, max_speed, max_acceleration
):
    """
    ...
    if d_const_speed < 0:
        t_acc = (
            -start_stop_speed + math.sqrt(start_stop_speed**2 + max_acceleration * d)
        ) / max_acceleration
    ...
    t = np.hstack(
        [
            np.arange(0, total_time, sampling_time),
    ...
    for i in range(1, t.size - 1):
        if t[i] < t_acc:
            ...
            point[:, i] = start_point + unit_dir * (
                d_acc
                + d_const_speed
                + max_speed * (t[i] - t_acc - t_const_speed)
                - 0.5 * max_acceleration * (t[i] - t_acc - t_const_speed) ** 2
            )
...
def trajectory_from_path_bang_bang(
    target_path, max_velocity, sampling_time, min_speed=0, max_acceleration=1
):
    """
