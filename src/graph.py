...
class Graph:
    ...
    def __init__(self, startposition, endposition, fix_room_size=True):
        ...
        # size of searchbox
        if fix_room_size:
            self.searchboxsize_x = 20.0  # (startposition[0] - endposition[0])
            self.searchboxsize_y = 10.0  # (startposition[1] - endposition[1])
            self.searchboxsize_z = 10.0  # (startposition[2] - endposition[2])
            # location of seachbox startpoint: two times size of box between start and end position
            self.searchbegin_x = 0.0
            self.searchbegin_y = 0.0
            self.searchbegin_z = 0.0
        else:
            ...
            # location of seachbox startpoint: two times size of box between start and end position
            self.searchbegin_x = self.startposition[0] + (self.searchboxsize_x / 4)
            self.searchbegin_y = self.startposition[1] + (self.searchboxsize_y / 4)
            self.searchbegin_z = self.startposition[2] + (self.searchboxsize_z / 4)
        ...
        self.distances = {0: 0.0}  # dictonary with distances for each index
        self.connections = {0: []}  # dictonary with connecting node(s)

    # creating a random position within search frame
    def randpos(
        self, obstacles=None, rand_radius=None, bias=None, obstacle_bias=False
    ) -> tuple:
        ...
        if obstacle_bias == False:
            ...
            if select > bias:
                ...
                # convert to value within searchbox
                posx = (
                    self.searchbegin_x + fix_room_factor * x * self.searchboxsize_x * 2
                )
                posy = (
                    self.searchbegin_y + fix_room_factor * y * self.searchboxsize_y * 2
                )
                posz = (
                    self.searchbegin_z + fix_room_factor * z * self.searchboxsize_z * 2
                )
            else:
                ...
                if obstacles[rand_obs][-1] == "sphere":
                    rand_pos = (direction / length) * (
                        obstacles[rand_obs][3] + uniform(0.0, rand_radius)
                    )
                ...
                if obstacles[rand_obs][-1] == "cube":
                    ...
                    length = np.linalg.norm(direction)
                    dimensions_cube = (np.array(obs[1]) - np.array(obs[0])) + uniform(
                        0.1, rand_radius
                    )
                    # center base_point, pick a direction, mulitply with width/2, height/2, depth/2 and add offset rand_radius
                    rand_pos = center + np.multiply(
                        (direction / length), dimensions_cube / 2
                    )
                    posx = rand_pos[0]
                    posy = rand_pos[1]
                    posz = rand_pos[2]
    ...
    def add_edge(self, start_node, end_node, cost):
        ...
        self.connections[start_node].append(
            (end_node, cost)
        )  # add the connecting nodes and costs
